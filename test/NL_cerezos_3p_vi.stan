...
parameters { 
  ...
  vector[K_scal] b_scal;  // population-level effects 
  vector[K_xmid] b_xmid;  // population-level effects 
...
model { 
  ...
  target += uniform_lpdf(b_xmid | 0, 10); 
  target += uniform_lpdf(b_Asym | 0.5, 10); 
...
generated quantities { 
  ...
  for (n in 1:N_Y) { 
      mu[n] = GDH_Y[n] * (mu_Asym[n] / (1 + exp((mu_xmid[n] - CP_Y[n]) / mu_scal[n])));
