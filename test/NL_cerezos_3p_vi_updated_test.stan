data { 
  ...
  real temp_Intercept_a;
} 
transformed data { 
} 
parameters { 
  vector<lower=0>[K_alpha] b_alpha;  // population-level effects 
  vector<lower=0>[K_eta] b_eta;  // population-level effects 
  vector<upper=0>[K_beta] b_beta;  // population-level effects 
  vector<lower=0>[M_1] sd_1;  // group-level standard deviations 
  vector[N_1] z_1[M_1];  // unscaled group-level effects 
  vector<lower=0>[M_2] sd_2;  // group-level standard deviations 
  vector[N_2] z_2[M_2];  // unscaled group-level effects 
  real temp_Intercept_b;
} 
...
model { 
  ...
  for (n in 1:N) { 
    ...
    target += ordered_logistic_lpmf(Y[n] | mu[n], [temp_Intercept_a,temp_Intercept_b]');
  } 
  
  target += uniform_lpdf(b_eta | 0, 10); 
  target += uniform_lpdf(b_beta | -10, 0); 
  target += uniform_lpdf(b_alpha | 0, 10); 
  ...
  target += uniform_lpdf(temp_Intercept_b | 4, 10); 
...
generated quantities { 
  ...
  for (n in 1:N) { 
    mu[n] = C_1[n] * (mu_alpha[n] / (1 + exp(-mu_beta[n] - C_2[n]*mu_eta[n])));
    Yrep[n] = ordered_logistic_rng(mu[n], [temp_Intercept_a,temp_Intercept_b]');
    Ysat[n] = -(mu_beta[n] - 2.292432)/mu_eta[n];
  } 
  
  for (n in 1:N_Y) { 
    mu_rep[n] = GDH_Y[n] * (mu_alpha[n] / (1 + exp(-mu_beta[n] - CP_Y[n]*mu_eta[n])));
    Yhat[n] = ordered_logistic_rng(mu[n], [temp_Intercept_a,temp_Intercept_b]');
  } 
} 
